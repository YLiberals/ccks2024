...
def extract_mentioned_entities_from_sparql(sparql:str) -> List[str]:
    ...
    toks = [_.split(' ') for _ in sparql.split('\n')]
    toks = [x.replace('\t.','') for tok in toks for x in tok if len(x)]
    entity_tokens = []
    for i in range(0, len(toks), 2):
        if toks[i].startswith('<') or toks[i].startswith('"'):
            entity_tokens.append(toks[i][1:-1])
