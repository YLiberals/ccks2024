'''
Author       : Zhijie Yang
Date         : 2024-05-17 02:25:23
LastEditors  : Zhijie Yang
LastEditTime : 2024-05-19 22:13:23
FilePath     : /race/step_3_parse_sparql.py
Description  : 
...
from components.utils import vanilla_sexpr_linearization_method

class ParseError(Exception):
    pass

gc = GstoreConnector('localhost','9000', 'root', '123456')
query = gc.query
...
class Parser:
    ...
    def parse_query_cwq(self, query, mid_list):
        ...
        assert next_line.startswith('SELECT ?x')
        line_num = line_num + 1
        next_line = lines[line_num]
        ...
        try:
            ...
            s_expr = 'null'
        return vanilla_sexpr_linearization_method(s_expr)
    ...
    def parse_naive_body(self, body_lines, filter_lines, ret_var, spec_condition=None):
        ...
        triplets = [[x.strip('<>') for x in tri] for tri in triplets]  # remove "<>"
...
def augment_with_s_expr_cwq(split, check_execute_accuracy=False):
    ...
    if split == 'train':
        dataset = load_json(f'data/race/format/train.json')
    elif split == 'test':
        dataset = load_json(f'data/race/format/test.json')
    total_num = 0
    hit_num = 0
    execute_hit_num = 0
    failed_instances = []
    ...
    for i,data in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        if flag_success:
            ...
            if check_execute_accuracy:
                ...
                try:
                    execute_ans = [s.replace('http://rdf.freebase.com/ns/','') if type(s) == str else str(s) for s in query(lisp_to_sparql(instance['SExpr']))]
                    if split=='test':
                        gold_ans = [s.replace('http://rdf.freebase.com/ns/','') if type(s) == str else str(s) for s in query(lisp_to_sparql(data['sparql']))]
                    else:
                    ...
                    if set(execute_ans) == set(gold_ans):
                        ...
                        temp = [s.replace('http://rdf.freebase.com/ns/','') if type(s) == str else str(s) for s in query(lisp_to_sparql(instance['SExpr']))]
    ...
    sexpr_dir = 'data/race/sexpr'
    if not os.path.exists(sexpr_dir):
        os.makedirs(sexpr_dir)
    dump_json(dataset, f'{sexpr_dir}/{split}.expr.json', indent=4)
    #dump_json(failed_instances, f'{sexpr_dir}/CWQ.{split}.expr_failed.json', indent=4)


def extract_entities_and_rel(clause:str) -> "List[str]":
    '''
...
def convert_cwq_sparql_instance(sparql, origin_data):
    ...
    pattern_str = r'\{.+\}'
    clauses = re.findall(pattern_str, sparql, re.DOTALL)[0].split('\n')
    mid_list = list(set([mid.strip()
                         for clause in clauses for mid in extract_entities_and_rel(clause)
                         if mid[0] == '<']))
