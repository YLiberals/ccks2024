...
def try_relation(d):
    ...
    similarities = model.similarity([("'s".join(_.split('.'))).replace("_", " ") for _ in rel_list], cand_rels)
    change = dict()
...
def aggressive_top_k_eval_new(split, predict_file, dataset):
    ...
    failed_preds_file = os.path.join(dirname,f'{filename}_gen_failed_results.json')
    if not os.path.exists(result_file):
        dump_json([], result_file, indent=4)
    if not os.path.exists(official_results_file):
        dump_json([], official_results_file, indent=4)
    if not os.path.exists(failed_preds_file):
        dump_json([], failed_preds_file, indent=4)
    ...
    lines = load_json(result_file)
    official_lines = load_json(official_results_file)
    failed_preds = load_json(failed_preds_file)

    st = len(lines) + len(failed_preds)
    predictions,gen_dataset = predictions[st:],gen_dataset[st:]
    for (pred,gen_feat) in tqdm(zip(predictions,gen_dataset), total=len(gen_dataset), desc=f'Evaluating {split}'):
        ...
        for rank, p in enumerate(pred['predictions'][:8]):
        ...
        dump_json(failed_preds,failed_preds_file,indent=4)
